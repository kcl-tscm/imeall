.. Imeall documentation master file, created by
   sphinx-quickstart on Mon Aug  8 12:35:13 2016.
   You can adapt this file completely to your liking, but it should at least
   contain the root `toctree` directive.

Welcome to Imeall's documentation!
==================================
...
Initialize the imeall app ``python runserver.py``
...
For instance /alphaFe/110 contains all computed 110 symmetric tilt 
grain boundaries for iron. The 000 orientation axes implies a perfect 
crystal, different cleavage planes and singular defects and vacancies 
...
tight binding models, EAMs, and Density Functional calculations. 
If multiple "flavours" of each calculation
type are to be used the directories can take more specific 
names e.g. an EAM paramterized by a particular worker, and two DFT 
calculations using different program might be labeled: EAM_Mishin, DFT_PW, DFT_VASP etc.
...
This could constitute a pattern like ``_d2.0`` which denotes:
...
The canonical grain contains a file ``gb.json``, and
subsequent directories each contain a file ``subgb.json``
...
* ``n_at``      : Number of atoms in the grain boundary simulation cell.
